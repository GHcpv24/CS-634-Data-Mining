...
st.markdown("<body style ='color:#00D9DC;'></body>", unsafe_allow_html=True)
...
st.markdown("<h5 style='text-align: center; color: #00D9DC;'>On the sidebar to the left, you will a series of questions of the house specifics that require your input. Once you have finished answering the questions, click on the below button to generate the predicted house sale price as well as respective SHAP summary plot and SHAP interaction plot.</h5>", unsafe_allow_html=True)

st.markdown("<h5 style='text-align: center; color: #00D9DC;'>Note: Please allow the app some time (a few seconds) to generate predictions and plots.</h5>", unsafe_allow_html=True)
...
if center_button:
    ...
    with col2:
        ...
        shap_explainer = explainer(data_df)
        shap_values = shap.TreeExplainer(lgbm_base).shap_values(data_df)
        shap_interaction_values = shap.TreeExplainer(lgbm_base).shap_interaction_values(data_df)

        st.subheader("SHAP Summary Plot")
        st.markdown("<div style='text-align: center;'>", unsafe_allow_html=True)
        shap.plots.beeswarm(shap_explainer, max_display=10)
        st.markdown("</div>", unsafe_allow_html=True)
        st.pyplot()

        st.subheader("SHAP Interaction Plot")
        st.markdown("<div style='text-align: center;'>", unsafe_allow_html=True)
        shap.summary_plot(shap_interaction_values, data_df)
        st.markdown("</div>", unsafe_allow_html=True)
        st.pyplot()
        ...
        shap_explainer = explainer(data_df)
        shap_values = shap.TreeExplainer(lgbm_opt).shap_values(data_df)
        shap_interaction_values = shap.TreeExplainer(lgbm_opt).shap_interaction_values(data_df)
